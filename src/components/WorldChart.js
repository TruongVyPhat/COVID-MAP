...
const WorldChart = ({}) => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            // data
            let data_tmp = DataJson.World_Data;
            ...
            for (let i = 0; i < apiData_size; i++){
                ...
                if (value_arr.length !== Object.keys(wPos).length){
                ...
                for (let j = 0; j < number_of_type; j++){
                    ...
                    let pos_of_case = wPos[label_of_case];             // lấy vị trí của từng trường hợp trong API array
    ...
    options.title.text = "World Covid Chart";
